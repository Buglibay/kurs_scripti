...
<!-- Breadcrumb section -->
    ...
    //retvive selected results from database  and display them on page 
     ...
     INNER JOIN blog_categories ON blog.blog_categories = blog_categories.categoria_id ORDER BY blog_id DESC LIMIT ".$this_page_fist_result.",".$results_per_page;
     $result=mysqli_query($conn,$sql);
...
<div class="post-item mt-5">
	...
	<div>
		<center>
			...
			<li><a href="blog.php?page=<?=1?>"><i>İLK</i></a></li>

			<li><a href="blog.php?page=<?=($page-1)?>"><i class="fa fa-angle-left"></i></a></li>
...
if($i>0  && $i<=$nubmer_of_pages){
	...
	if($i==$page){
		 ?> 
 		 <li><a href="blog.php?page=<?=$i?>"class="active"><?=$i?></a></li> 
		<?php 	 
}else{ ?> <li><a href="blog.php?page=<?=$i?>"><?=$i?></a></li> <?php }
}
		   
	    } ?> 
	    
       <li><a href="blog.php?page=<?=($page+1)?>"><i class="fa fa-angle-right"></i></a></li>
       <li><a href="blog.php?page=<?=$nubmer_of_pages?>"><i>SON</i></a></li>
     </ul>
